...
export function useRelatorioViewModel() {
  ...
  const estatisticas = {
    ...
    faturamentoTotal: registrosFiltrados.reduce((acc, curr) => {
      if (curr.valorFinal) return acc + curr.valorFinal;

      if (curr.dataEntrada && curr.dataSaida) {
        const calc = MovimentacaoModel.calcularValor(
          curr.dataEntrada,
          curr.dataSaida,
          curr.cpf,
        );
        return acc + (calc.valor || 0);
      }
    ...
    tempoMedio: (() => {
      ...
      const totalMinutos = concluidos.reduce(
        (acc, curr) =>
          acc +
          MovimentacaoModel.calcularTempoPermanencia(
            curr.dataEntrada,
            curr.dataSaida,
          ),
        0,
