...
const empresasMock = [
  ...
  { id: 2, nome: "Logística TI", status: "Ativo" },
];
...
export function useMotoristaViewModel() {
  ...
  const salvarMotorista = async (dados) => {
    ...
    try {
      ...
      const ConvEncontrado = conveniosMock.find(
      ...
      if (!modal.dado) {
        if (motoristas.some((m) => m.cpf === dados.cpf))
          throw new Error("CPF já cadastrado.");
        setMotoristas((prev) => [
          ...prev,
          { id: Date.now(), ...payload, status: "Ativo" },
        ]);
        setMensagem({
          mostrar: true,
        ...
        setMotoristas((prev) =>
          prev.map((m) => (m.id === modal.dado.id ? { ...m, ...payload } : m)),
        );
        setMensagem({
          mostrar: true,
